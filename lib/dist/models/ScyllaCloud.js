...
function licensePrice(cluster, licensePriceCore) {
...
function reservedPrice(cluster) {
...
function itemSizePerfFactor(itemSize) {
    if (itemSize <= 10) {
        return 1;
    }
...
*/
function selectClusterConfigs(specs) {
    return instanceTypes.aws.map(instanceType => {
        const nodes = _.find(_.range(1, 300), n => (instanceType.vcpu * n >= specs.vcpu &&
            instanceType.memory * n >= specs.memory &&
            instanceType.storage * n >= specs.storage)) || 0;
        return { instanceType, nodes };
    }).filter(({ nodes }) => nodes > 0);
}
export function selectClusterInstances(workload, replicationFactor, perf) {
    ...
    const lowestPrice = _.chain(minimalConfigs)
        ...
        .min()
        .value();
    const bestConfig = (configs) => _.chain(configs)
...
export function clusterCapacity(cluster, replicationFactor, perfMode = MODE.CQL) {
    const perf = vcpuPerf[perfMode];
    const totalResources = clusterResources(cluster);
    const dataset = totalResources.storage / replicationFactor / CompactionOverhead;
    const peakLoad = (totalResources.vcpu * (perf.writes + perf.reads)) /
        2 /
        replicationFactor;
    const sustainedLoad = peakLoad * 0.66;
    return { sustainedLoad, peakLoad, dataset, ...totalResources };
}
export function prices(workload, replicationFactor, perfMode = MODE.CQL) {
    const perf = vcpuPerf[perfMode];
    // currently, Scylla requires each replica to be in a different AZ
    const replicationTraffic = hoursPerMonth * 3600 * ((workload.reads + workload.writes) *
        workload.itemSize *
        (replicationFactor - 1)) /
...
//# sourceMappingURL=ScyllaCloud.js.map